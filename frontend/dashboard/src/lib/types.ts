...
export interface Factor { key: string; label: string; value: string; raw: number | null; score: number | null; weight: number; note?: string }
export interface ComponentScore { key: ComponentKey; label: string; weight: number; score: number | null; factors: Factor[]; note?: string }
...
export interface RiskEvent { id: string; slug: string; ts: string; kind: 'liquidity' | 'activity' | 'collateral' | 'transparency' | 'score'; severity: 'info' | 'watch' | 'alert'; message: string; delta: number | null }
...
export interface Snapshot {
  ...
  market: { protocolsTracked: number; totalTvlUsd: number; totalTvlChange7d: number | null; totalTvlChange30d: number | null; activeAddresses7d: number; tx7d: number; avgScore: number; bandDistribution: Record<Band, number>; ecosystemTvlHistory: TvlPoint[] };
